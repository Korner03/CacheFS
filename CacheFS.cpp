...
int CacheFS_init(int blocks_num, cache_algo_t cache_algo, double f_old , double f_new) {
    ...
    if (cache_algo == cache_algo_t::FBR &&
            (f_old < 0 || f_old > 1 || f_new < 0 || f_new > 1 || f_old + f_new > 1))
        return 1;

    try {
        switch (cache_algo) {
            case cache_algo_t::LRU:
                g_cache_fs = new LRU_Container(blocks_num);
                break;
            case cache_algo_t::LFU:
                g_cache_fs = new LFU_Container(blocks_num);
                break;
            case cache_algo_t::FBR:
                g_cache_fs = new FBR_Container(f_old, f_new, blocks_num);
                break;
        }
    } catch (exception) {
        return 1;
...
int CacheFS_open(const char *pathname) {
    ...
    char *actual_path = realpath(pathname, NULL);
    ...
    free(actual_path);
    return g_cache_fs->open_file(pathname);
}
...
int CacheFS_close(int file_id) {
    return g_cache_fs->close_file(file_id);
}
...
int CacheFS_pread(int file_id, void *buf, size_t count, off_t offset) {
    return g_cache_fs->fs_pread(file_id, buf, count, offset);
}
...
int CacheFS_print_cache (const char *log_path) {
    return g_cache_fs->print_cache(log_path);
}
...
int CacheFS_print_stat (const char *log_path) {
    return g_cache_fs->print_stat(log_path);
}
